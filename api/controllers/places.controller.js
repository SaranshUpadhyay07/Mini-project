...
const FILE = "api/controllers/places.controller.js";
const log = (fn, message) => console.log(`[${FILE}] [${fn}] ${message}`);
const logError = (fn, message) => console.error(`[${FILE}] [${fn}] ${message}`);

/**
 * Search for pilgrim locations in a specific city using Mappls API
 */
export const searchPilgrimLocations = async (req, res) => {
  const FN = "searchPilgrimLocations";
  log(FN, "entry");

  try {
    const { city } = req.query;
    log(FN, `query city=${city || "N/A"}`);

    if (!city) {
      log(FN, "branch missing_city");
    ...
    log(FN, `prepared queries count=${searchQueries.length}`);

    // Perform multiple searches to get comprehensive results
    for (const query of searchQueries) {
      try {
        log(FN, `mappls_search_call query="${query}"`);
        ...
        const suggestedCount = response?.data?.suggestedLocations?.length || 0;
        log(
          FN,
          `mappls_search_result query="${query}" suggested=${suggestedCount}`,
        );
        ...
        logError(
          FN,
          `mappls_search_error query="${query}": ${searchError.message}`,
        );
    ...
    log(
      FN,
      `result filtered=${pilgrimLocations.length} raw=${allLocations.length}`,
    );
    ...
    logError(FN, `error: ${error?.message || error}`);
...
export const getLocationDetails = async (req, res) => {
  const FN = "getLocationDetails";
  log(FN, "entry");

  try {
    const { eLoc } = req.params;
    log(FN, `params eLoc=${eLoc || "N/A"}`);

    if (!eLoc) {
      log(FN, "branch missing_eloc");
      return res.status(400).json({
        success: false,
    ...
    log(FN, `mappls_geocode_call eLoc=${eLoc}`);
    const response = await axios.get(
    ...
    const hasResult = Boolean(response?.data?.copResults);
    log(FN, `mappls_geocode_result hasResult=${hasResult}`);
    ...
    if (response.data && response.data.copResults) {
      ...
      log(FN, `branch not_found eLoc=${eLoc}`);
      res.status(404).json({
        success: false,
...
export const getOdishaPilgrimLocations = async (req, res) => {
  const FN = "getOdishaPilgrimLocations";
  log(FN, "entry");
  ...
  try {
    ...
    const searchLocations = [
      {
        ...
        lat: 19.8135,
        lng: 85.8312,
        keywords: "temple;shrine;pilgrimage",
      },
      {
        city: "Bhubaneswar",
        lat: 20.2961,
        lng: 85.8245,
        keywords: "temple;shrine;religious",
      },
      {
        city: "Konark",
        lat: 19.8876,
        lng: 86.0945,
        keywords: "temple;monument",
      },
    ];
    log(FN, `prepared nearby locations count=${searchLocations.length}`);
    ...
    for (const location of searchLocations) {
      try {
        log(
          FN,
          `mappls_nearby_call city=${location.city} ref=${location.lat},${location.lng}`,
        );
        ...
        log(
          FN,
          `mappls_nearby_result city=${location.city} suggested=${suggestedCount}`,
        );
        ...
        logError(
          FN,
          `mappls_nearby_error city=${location.city}: ${cityError.message}`,
        );
        console.error(
          `Error in nearby search for ${location.city}:`,
          cityError.message,
        );
      }
    }
    ...
    if (allLocations.length === 0) {
      log(FN, "branch fallback_locations");
      ...
      const fallbackLocations = [
        ...
        },
        {
          ...
          type: "Temple",
          eLoc: "MMI001",
          address: "Lingaraj Nagar, Bhubaneswar",
        },
        {
        ...
        },
        {
          ...
          type: "Temple",
          eLoc: "MMI004",
          address: "Tankapani Road, Bhubaneswar",
        },
        {
    ...
    log(FN, `sorting results count=${allLocations.length}`);
